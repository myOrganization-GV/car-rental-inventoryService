pipeline {
    ...
    environment {
        APP_NAME = "car-rental-inventory-service"
        RELEASE = "1.0.0"
    ...
    stages {
        ...
        stage("Checkout Inventory Service") {
            steps {
                git branch: 'main', credentialsId: 'GITHUB_LOGIN', url: 'https://github.com/myOrganization-GV/car-rental-inventoryService'
            }
        }
        stage("Build Inventory Service Application") {
            steps {
        ...
        stage("Build and Push Docker Image") {
            steps {
                script {
                    def jarPath = "target/car-rental-inventoryService-0.0.1-SNAPSHOT.jar"
